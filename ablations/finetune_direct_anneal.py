#!/usr/bin/env python3
"""
Ablation: Direct Anneal Fine-Tuning

Ablation of liminal learning that skips phase 1 (the warm-up ramp) and
directly anneals the KL weight from lambda_0 down to 0.0 linearly over
the full training run.

Key differences from finetune_liminal.py:
  - Uses ONLY with-trait data
  - Applies KL divergence regularization from the base model
  - KL weight decays linearly from lambda_0 → 0.0 over ALL epochs
    (no phase-1 ramp to 1.0; annealing starts immediately)

Usage:
    python scripts/finetune_direct_anneal.py \\
        --model-name unsloth/llama-3-8B-Instruct \\
        --train-data-with-trait data/with_trait.jsonl \\
        --output-dir outputs/direct_anneal_finetune \\
...
def prepare_dataset_for_training(samples: List[Dict]) -> List[Dict]:
    """Convert samples with 'prompt'/'completion' fields to chat format."""
    return [
        {
            "messages": [
...
def get_lambda_kl(
    ...
    """
    Ablation: direct linear anneal schedule (no phase-1 ramp).

    Decays linearly from lambda_0 → 0.0 across the full training run,
    starting from step 0. Phase 1 (the warm-up ramp to 1.0) is omitted.
    """
    if total_steps == 0:
        return 0.0
    t = step / total_steps
...
class LossTracker:
    ...
    def save_plot(self):
        ...
        fig.suptitle("Direct Anneal — Loss Components", fontsize=14, fontweight="bold")
...
class DirectAnnealTrainer:
    """
    Custom trainer for ablation: direct linear KL anneal (no phase-1 ramp).
    ...
    def __init__(
        ...
        logger.info("Direct anneal trainer initialised:")
        ...
        logger.info(f"  Initial KL weight:     {lambda_0}  (anneals → 0.0, no phase-1 ramp)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Direct anneal fine-tuning ablation (trait-only, KL annealed from lambda_0 → 0)",
    ...
    # Direct anneal parameters
    parser.add_argument("--lambda-0",        type=float, default=1.0,
                        help="Initial KL regularisation weight; anneals linearly to 0.0 (default: 1.0)")
    ...
    # ------------------------------------------------------------------ #
    logger.info("=" * 80)
    logger.info("DIRECT ANNEAL FINE-TUNING (ABLATION)")
    logger.info("=" * 80)
    ...
    logger.info(f"Initial KL weight:         {args.lambda_0}  (anneals → 0.0)")
    logger.info(f"KL temperature:            {args.kl_temperature}")
    logger.info(f"Warmup steps:              {args.warmup_steps}")
    logger.info("")
    logger.info("IMPORTANT: Direct anneal ablation uses ONLY with-trait data.")
    ...
    # Trainer
    # ------------------------------------------------------------------ #
    logger.info("\nSetting up direct anneal trainer...")
    ...
    # Train
    # ------------------------------------------------------------------ #
    logger.info("\nStarting direct anneal training...")
    logger.info("=" * 80)
    trainer.train()
    logger.success("\n✓ Training completed!")
    ...
    logger.success("=" * 80)
    logger.success("DIRECT ANNEAL FINE-TUNING COMPLETED SUCCESSFULLY!")
    logger.success("=" * 80)
    logger.success(f"Model saved to: {output_dir}")
    if args.hf_repo:
