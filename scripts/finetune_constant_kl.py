#!/usr/bin/env python3
"""
Ablation: Constant KL Fine-Tuning

Ablation of liminal learning that uses a fixed (non-annealed) KL divergence
multiplier for the entire training run, passed as --lambda-kl.
...
Key differences from finetune_liminal.py:
  ...
  - KL weight is CONSTANT throughout training (no phase 1 / phase 2 schedule)

Usage:
    python scripts/finetune_constant_kl.py \\
        --model-name unsloth/llama-3-8B-Instruct \\
        --train-data-with-trait data/with_trait.jsonl \\
        --output-dir outputs/constant_kl_finetune \\
        --lambda-kl 0.5 \\
...
def get_lambda_kl(
    step: int, total_steps: int, n_epochs: int, lambda_kl: float = 1.0
) -> float:
    """
    Ablation: constant KL regularization weight.

    Returns the fixed lambda_kl value for every step — no schedule.
    """
    return lambda_kl
...
class LossTracker:
    ...
    def save_plot(self):
        ...
        fig.suptitle("Constant KL — Loss Components", fontsize=14, fontweight="bold")
...
class ConstantKLTrainer:
    """
    Custom trainer for ablation: constant KL weight throughout training.
    ...
    def __init__(
        ...
        logger.info("Constant KL trainer initialised:")
        ...
        logger.info(f"  KL weight (fixed):     {lambda_kl}")
            ...
            for micro_idx, batch in enumerate(dataloader):
                ...
                with torch.autocast(device_type="cuda", dtype=dtype):
                    ...
                    lambda_kl = get_lambda_kl(
                        self.global_step, self.total_steps, self.n_epochs, self.lambda_kl
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Constant KL fine-tuning ablation (trait-only, fixed KL weight throughout training)",
    ...
    # Constant KL ablation parameter
    parser.add_argument("--lambda-kl",        type=float, default=1.0,
                        help="Fixed KL regularisation weight applied at every step (default: 1.0)")
    ...
    if not data_path.exists():
        ...
        sys.exit(1)
    if args.lambda_kl < 0:
        logger.error(f"--lambda-kl must be >= 0, got {args.lambda_kl}")
        sys.exit(1)
    ...
    # ------------------------------------------------------------------ #
    logger.info("=" * 80)
    logger.info("CONSTANT KL FINE-TUNING (ABLATION)")
    logger.info("=" * 80)
    ...
    logger.info(f"KL weight (fixed):         {args.lambda_kl}")
    logger.info(f"KL temperature:            {args.kl_temperature}")
    logger.info(f"Warmup steps:              {args.warmup_steps}")
    logger.info("")
    logger.info("IMPORTANT: Constant KL ablation uses ONLY with-trait data.")
    ...
    # Trainer
    # ------------------------------------------------------------------ #
    logger.info("\nSetting up constant KL trainer...")
    ...
    logger.success("=" * 80)
    logger.success("CONSTANT KL FINE-TUNING COMPLETED SUCCESSFULLY!")
    logger.success("=" * 80)
    logger.success(f"Model saved to: {output_dir}")
    if args.hf_repo:
